...
//    float getResistance();
    float getResistance(float tempRLOAD, float analogExt, int ext);
//    float getCorrectedResistance(float t, float h);
    float getCorrectedResistance(float tempRLOAD, float analogExt, int ext, float temperature, float humidity);
//    float getPPM();
    float getPPM(float tempRLOAD, float tempRZERO, float analogExt, int ext);
//    float getCorrectedPPM(float t, float h);
    float getCorrectedPPM(float tempRLOAD, float tempRZERO, float analogExt, int ext, float temperature, float humidity);
//    float getRZero();
    float getRZero(float tempRLOAD, float tempATMOCO2, float analogExt, int ext);
//    float getCorrectedRZero(float t, float h);
    float getCorrectedRZero(float tempRLOAD, float tempATMOCO2, float analogExt, int ext, float temperature, float humidity);
};
//#endif
...
*/
float MQ135::getResistance(float tempRLOAD, float analogExt, int ext) {
  if (ext) {
      return (3.3/analogExt - 1.)*tempRLOAD;
  } else {
      int val = analogRead(_pin);
...
*/
float MQ135::getCorrectedResistance(float tempRLOAD, float analogExt, int ext, float temperature, float humidity) {
  return getResistance(tempRLOAD, analogExt, ext)/getCorrectionFactor(temperature, humidity);
}
...
*/
float MQ135::getPPM(float tempRLOAD, float tempRZERO, float analogExt, int ext) {
  return PARA * pow((getResistance(tempRLOAD, analogExt, ext)/tempRZERO), -PARB);
}
...
*/
float MQ135::getCorrectedPPM(float tempRLOAD, float tempRZERO, float analogExt, int ext, float temperature, float humidity) {
  return PARA * pow((getCorrectedResistance(tempRLOAD, analogExt, ext, temperature, humidity)/tempRZERO), -PARB);
}
...
*/
float MQ135::getRZero(float tempRLOAD, float tempATMOCO2, float analogExt, int ext) {
  return getResistance(tempRLOAD, analogExt, ext) * pow((tempATMOCO2/PARA), (1./PARB));
}
...
*/
float MQ135::getCorrectedRZero(float tempRLOAD, float tempATMOCO2, float analogExt, int ext, float temperature, float humidity) {
  return getCorrectedResistance(tempRLOAD, analogExt, ext, temperature, humidity) * pow((tempATMOCO2/PARA), (1./PARB));
}
...
boolean Plugin_135(byte function, struct EventStruct *event, String& string)
{
  ...
  switch (function)
  {
    ...
    case PLUGIN_WEBFORM_LOAD:
      {
        ...
        addFormCheckBox(F("Enable external analog input"), F("plugin_135_enable_external_analog"), Settings.TaskDevicePluginConfig[event->TaskIndex][5]);
        addFormNote(F("If this is enabled, analog input source below need to be configured."));
        // analog input
        addHtml(F("<TR><TD>Analog Input:<TD>"));
        addTaskSelect(F("plugin_135_analog_input"), Settings.TaskDevicePluginConfig[event->TaskIndex][6]);
        LoadTaskSettings(Settings.TaskDevicePluginConfig[event->TaskIndex][6]); // we need to load the values from another task for selection!
    ...
    case PLUGIN_WEBFORM_SAVE:
      {
        ...
        Settings.TaskDevicePluginConfig[event->TaskIndex][5] = isFormItemChecked(F("plugin_135_enable_external_analog") );
        Settings.TaskDevicePluginConfig[event->TaskIndex][6] = getFormItemInt(F("plugin_135_analog_input"));
    ...
    case PLUGIN_INIT:
      {
        ...
        Plugin_135_MQ135[event->TaskIndex]->analogExt = 0;
        Plugin_135_MQ135[event->TaskIndex]->ext = 0;
    ...
    case PLUGIN_READ:
        ...
        if (Settings.TaskDevicePluginConfig[event->TaskIndex][5]) {
            // we're checking a var from another task, so calculate that basevar
            byte TaskIndex3    = Settings.TaskDevicePluginConfig[event->TaskIndex][6];
            byte BaseVarIndex3 = TaskIndex3 * VARS_PER_TASK;
            Plugin_135_MQ135[event->TaskIndex]->analogExt = UserVar[BaseVarIndex3]; // in Volt
            Plugin_135_MQ135[event->TaskIndex]->ext = 1;
        } else {
            Plugin_135_MQ135[event->TaskIndex]->analogExt = 0;
            Plugin_135_MQ135[event->TaskIndex]->ext = 0;
        }

        float analogExt = Plugin_135_MQ135[event->TaskIndex]->analogExt;
        int ext = Plugin_135_MQ135[event->TaskIndex]->ext;
        ...
        if (Settings.TaskDevicePluginConfig[event->TaskIndex][0]) {
            ...
            UserVar[event->BaseVarIndex] = Plugin_135_MQ135[event->TaskIndex]->getCorrectedPPM(tempRLOAD, tempRZERO, analogExt, ext, temperature, humidity);
            UserVar[event->BaseVarIndex + 2] = Plugin_135_MQ135[event->TaskIndex]->getCorrectedRZero(tempRLOAD, tempATMOCO2, analogExt, ext, temperature, humidity);
        } else {
            UserVar[event->BaseVarIndex] = Plugin_135_MQ135[event->TaskIndex]->getPPM(tempRLOAD, tempRZERO, analogExt, ext);
            UserVar[event->BaseVarIndex + 2] = Plugin_135_MQ135[event->TaskIndex]->getRZero(tempRLOAD, tempATMOCO2, analogExt, ext);
        }
        ...
        addLog(LOG_LEVEL_INFO, log);
        log = F("MQ135: External Analog Enable: ");
        log += ext;
        addLog(LOG_LEVEL_INFO, log);
        log = F("MQ135: Analog Value: ");
        log += analogExt;
        addLog(LOG_LEVEL_INFO, log);
